...
public class UnknownTypeHandler extends BaseTypeHandler<Object> {
  ...
  private TypeHandler<?> resolveTypeHandler(Object parameter, JdbcType jdbcType) {
    TypeHandler<?> handler;
    if (parameter == null) {//如果参数为空则采用Object类型处理器
      handler = OBJECT_TYPE_HANDLER;
    } else {
      handler = typeHandlerRegistrySupplier.get().getTypeHandler(parameter.getClass(), jdbcType);//从typeHandlerRegistrySupplier中获取对应的类型处理器
      // check if handler is null (issue #270)
      if (handler == null || handler instanceof UnknownTypeHandler) {//如果没有找到该类型的处理器则采用OBJECT_TYPE_HANDLER
        handler = OBJECT_TYPE_HANDLER;
      }
    }
    return handler;
