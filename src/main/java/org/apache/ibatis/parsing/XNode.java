...
public class XNode {
    ...
    while (current != null) {
      ...
      String value = current.getStringAttribute("id",
          current.getStringAttribute("value",
              current.getStringAttribute("property", (String) null)));//优先级：ID>value>property
      if (value != null) {//如果是ResultMap下的association标签则转化结果ResultMap[BaseResultMap]_association[book]
        value = value.replace('.', '_');//将原先的id（或者value和property）中的"."替换为_
        builder.insert(0, "]");
        builder.insert(0,
            value);
        builder.insert(0, "[");
  ...
  public String getStringAttribute(String name, String def) {//如果该属性找不到则返回传入的第二个参数
    String value = attributes.getProperty(name);
    if (value == null) {
      return def;
    } else {
  ...
  public Properties getChildrenAsProperties() {
    ...
    for (XNode child : getChildren()) {//遍历某个标签下的所有子标签中的属性值并将其放入properties中
