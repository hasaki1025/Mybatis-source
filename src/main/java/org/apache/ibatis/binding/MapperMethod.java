...
public class MapperMethod {
  ...
  public Object execute(SqlSession sqlSession, Object[] args) {
    Object result;
    switch (command.getType()) {//根据SQL语句类型跳转
      case INSERT: {
      ...
      case SELECT:
        if (method.returnsVoid() && method.hasResultHandler()) {//是否返回空值
          executeWithResultHandler(sqlSession, args);
          result = null;
        } else if (method.returnsMany()) {//是否返回多个值（数组类型或者集合类型）
          result = executeForMany(sqlSession, args);
        } else if (method.returnsMap()) {//是否返回Map集合
          result = executeForMap(sqlSession, args);
        } else if (method.returnsCursor()) {//是否返回游标
          result = executeForCursor(sqlSession, args);
        } else {//返回普通POJO
          Object param = method.convertArgsToSqlCommandParam(args);//使用对象包装所有参数
    ...
    if (result == null && method.getReturnType().isPrimitive() && !method.returnsVoid()) {//如果返回结果为空且Mapper方法返回类型为原始类型且Mapper方法返回类型不是Void则抛出异常
  ...
  public static class SqlCommand {
    ...
    private MappedStatement resolveMappedStatement(Class<?> mapperInterface, String methodName,
      ...
      if (configuration.hasStatement(statementId)) {
        return configuration.getMappedStatement(statementId);//从configuration中的mappedStatements获取MappedStatement
